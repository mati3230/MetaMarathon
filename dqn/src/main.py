...
def deep_q_learning(sess=None,
    ...
    for i in range(FLAGS.replay_memory_init_size):
        ...
        # TODO 3 choose a random action with probability of action_probs (Hint: use np.random.choice)
        # TODO 4 apply the action to make a step
        # TODO 5 preprocess the new state image
        # append the next state image on last position in stack
        next_state = np.append(state[:,:,1:], np.expand_dims(next_state, 2), axis=2)
        # save the transition in memory
        replay_memory.append(Transition(state, action, reward, next_state, done))
        if done: # game over
            # TODO 6 reset the environment
            # TODO 7 apply preprocessing
            state = np.stack([state] * 4, axis=2)
        else:
            # the next state will be the current state in the next iteration
            # TODO 8 set the state for the next iteration
            print("set the state for the next iteration")
    # record videos
    ...
    for i_episode in range(FLAGS.num_episodes):
        # TODO 9 save the current checkpoint with saver
        # TODO 10 reset the environment
        # TODO 11 apply preprocessing to state
        state = np.stack([state] * 4, axis=2)
        loss = None
        ...
        for t in itertools.count():
            ...
            # TODO 12 add episode_summary to q_estimator.summary_writer, use total_t for global step
            ...
            # TODO 13 choose a random action with probability of action_probs (Hint: use np.random.choice)
            # TODO 14 apply the action to make a step
            # TODO 15 apply preprocessing
            next_state = np.append(state[:,:,1:], np.expand_dims(next_state, 2), axis=2)
            ...
            # TODO 16 calculate q_values_next from target_estimator.predict, use next_states_batch for the input
            ...
            # TODO 17 update the q_estimator
            if done:
                break
            # TODO 18 set the state for the next iteration
            total_t += 1
...
def bot_play(sess=None,
    ...
    """
    # TODO 19 reset the environment
    # TODO 20 preprocess the state image
    # combine 4 images to a state 
    state = np.stack([state] * 4, axis=2)
    sum_reward = 0
    ...
    for episode in range(num_episodes):
        while(True):
            # TODO 21 render a frame from the environment
            # calculate q values and targets
            action_probs = policy(sess, state, 0.1)
            # TODO 22 choose a random action with probability of action_probs (Hint: use np.random.choice)
            # TODO 23 apply the action to make a step
            # TODO 24 preprocess the state image
            sum_reward += reward
            # append the next state image on last position in stack
            next_state = np.append(state[:,:,1:], np.expand_dims(next_state, 2), axis=2)
            
            if done: # game over
                # TODO 25 reset the environment
                # TODO 26 preprocess the state image
                state = np.stack([state] * 4, axis=2)
                print("episode: {0}, reward: {1}".format(episode, sum_reward))
                break
            else:
                # the next state will be the current state in the next iteration
                # TODO 27 set the state for the next iteration
                print("set the state for the next iteration")
