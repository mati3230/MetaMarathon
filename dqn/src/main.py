...
def deep_q_learning(sess=None,
    ...
    # TODO 1 reset the environment with state = env.reset()
    # preprocess the game image
    ...
    for i in range(FLAGS.replay_memory_init_size):
        ...
        # apply the action to make a step
        # TODO 2 apply the action to make a step with "next_state, reward, done, _ = env.step(valid_actions[action])"
        # preprocess the new state image
        ...
        if done: # game over
            # TODO 3 reset the environment with "state = env.reset()"
            # apply preprocessing
            state = state_processor.process(sess, state)
            state = np.stack([state] * 4, axis=2)
        else:
    ...
    for i_episode in range(FLAGS.num_episodes):
        # save the current checkpoint with saver
        saver.save(tf.get_default_session(), checkpoint_path)
        # TODO 4 reset the environment with "state = env.reset()"
        # apply preprocessing
        state = state_processor.process(sess, state)
        state = np.stack([state] * 4, axis=2)
        loss = None
        ...
        for t in itertools.count():
            ...
            # choose a random action with probability of action_probs
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            # TODO 5 apply the action to make a step with "next_state, reward, done, _ = env.step(valid_actions[action])"
            next_state = state_processor.process(sess, next_state)
            next_state = np.append(state[:,:,1:], np.expand_dims(next_state, 2), axis=2)
...
def bot_play(sess=None,
    ...
    """
    # TODO 6 reset the environment with "state = env.reset()"
    ...
    for episode in range(num_episodes):
        while(True):
            # TODO 7 call "env.render()" in order to render a frame
            # calculate q values and targets
            action_probs = policy(sess, state, 0.1)
            # choose a random action with probability of action_probs
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            # TODO 8 apply the action to make a step with "next_state, reward, done, _ = env.step(valid_actions[action])"
            # preprocess the state image
            next_state = state_processor.process(sess, next_state)
            sum_reward += reward
            # append the next state image on last position in stack
            ...
            if done: # game over
                # TODO 9 reset the environment with "state = env.reset()"
                # preprocess the state image
                state = state_processor.process(sess, state)
                state = np.stack([state] * 4, axis=2)
